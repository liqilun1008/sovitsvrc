...
```mermaid
graph TD
    A[1. 歌曲下载<br>下载多首无损音频 .wav, .flac] --> B{2. 人声分离<br>使用UVR5等工具移除伴奏与和声};
    B --> C[得到干净的人声干声];
    C --> D{3. 音频切片<br>切分为小于30s的片段并移除静音部分};
    D --> E[最终的训练数据集<br>(一组短音频文件)];
```
...
## 2. PM (Probabilistic Monaural F0 tracker)
...
## 3. RMVPE (Robust Monaural Vocal Pitch Estimator)
